...
from src.gp.gp_utils import safe_mknode_recursive, make_bz_matrix

import joblib
import os
from src.gp.gp_utils import get_model_dir_and_file

def restore_gp_regressors(pre_trained_models, mhe=False):
    """
    ...
    if isinstance(pre_trained_models['models'][0], dict):
        ...
        for _, model_dict in enumerate(pre_trained_models['models']):
            if x_features is not None:
                gp_reg = CustomGPRegression(x_features, u_features, model_dict["reg_dim"])
            else:
                gp_reg = CustomGPRegression(model_dict["x_features"], model_dict["u_features"], model_dict["reg_dim"])
        ...
        for key in np.sort(list(pre_trained_gp_reg.keys())):
            gp_reg_ensemble.add_model(pre_trained_gp_reg[key], mhe=mhe)
    else:
...
class CustomGPRegression:
    ...
    def predict(self, x_test, return_std=False, return_cov=False):
        ...
        # as currently gives negative covariances so cannot sqrt
...
class GPEnsemble:
    ...
    def add_model(self, gp, mhe=False):
        """"
        ...
        :type gp: list
        """
        x_dims = 9 if mhe else 13
        u_dims = 0 if mhe else 4
        ...
        self.B_z_dict[gp_dim] = make_bz_matrix(x_dims=x_dims, u_dims=u_dims, x_feats=gp[0].x_features, u_feats=gp[0].u_features)
